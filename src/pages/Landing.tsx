...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-bg-primary text-text-primary overflow-x-hidden">
      ...
      {/* Nav */}
      <nav
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-bg-primary/90 backdrop-blur-xl border-b border-border-primary' : ''}`}
      >
        ...
        <div className="max-w-[1200px] mx-auto">
          ...
          <div className="max-w-2xl mb-8 sm:mb-12">
            ...
            <p className="text-base sm:text-lg text-text-secondary leading-relaxed max-w-lg">
              GanttSmart turns your Linear projects into an interactive timeline you can actually drag around. No setup,
              no configuration — connect and go.
            </p>
          </div>
            ...
            {[
              {
                num: '01',
                title: 'Connect',
                desc: 'Sign in and authorize with Linear OAuth. Takes about ten seconds. Your tokens are stored encrypted — never in the browser.',
              },
              {
                num: '02',
                title: 'Visualize',
                desc: 'Pick a project. Every issue with a due date becomes a bar on the timeline. Start dates, dependencies, milestones — all there.',
              },
              {
                num: '03',
                title: 'Manage',
                desc: 'Drag bars to reschedule. Click to cycle status. Filter, group, zoom, export. Changes sync back to Linear instantly.',
              },
            ].map((step, i) => (
              <div
                key={i}
                className="py-8 lg:py-0 lg:px-8 border-b lg:border-b-0 lg:border-r border-border-primary last:border-0"
              >
        ...
        <div className="max-w-[1200px] mx-auto space-y-20">
          ...
          <div className="flex flex-col lg:flex-row items-start gap-8 lg:gap-16">
            <div className="flex-1 max-w-md">
              ...
              <p className="text-sm text-text-secondary leading-relaxed mb-4">
                Grab any bar edge to change due dates, drag the left edge for start dates, or move the entire bar to
                shift both. Every change writes back to Linear via API.
              </p>
              ...
              <div className="space-y-3">
                {['AUTH-12 → Mar 24', 'API-45 → Apr 2', 'DB-23 → Mar 28'].map((label, i) => (
                  <div key={i} className="flex items-center gap-3">
                    <div
                      className={`h-5 rounded ${i === 0 ? 'bg-urgent' : i === 1 ? 'bg-high' : 'bg-medium'} cursor-grab`}
                      style={{ width: `${50 + i * 15}%` }}
                    />
          ...
          <div className="flex flex-col lg:flex-row-reverse items-start gap-8 lg:gap-16">
            <div className="flex-1 max-w-md">
              ...
              <p className="text-sm text-text-secondary leading-relaxed">
                Filter by assignee, status, priority. Group by team member to see workload. Type to search across all
                task titles. Priority chips toggle on/off instantly.
              </p>
            </div>
            <div className="flex-1 rounded-lg border border-border-primary bg-bg-card p-4">
              <div className="flex flex-wrap gap-1.5 mb-3">
                {['Urgent', 'High', 'Medium', 'Low'].map((p, i) => (
                  <div
                    key={i}
                    className={`px-2.5 py-1 rounded-full text-[10px] font-medium border ${i < 3 ? 'border-accent/30 text-accent bg-accent/8' : 'border-border-secondary text-text-muted'}`}
                  >
                    {p}
                  </div>
                ))}
              ...
              <div className="flex gap-2">
                <div className="flex-1 px-2.5 py-1.5 rounded bg-bg-primary border border-border-primary text-[10px] text-text-muted">
                  All Assignees
                </div>
                <div className="flex-1 px-2.5 py-1.5 rounded bg-bg-primary border border-border-primary text-[10px] text-text-muted">
                  All Statuses
          ...
          <div className="flex flex-col lg:flex-row items-start gap-8 lg:gap-16">
            <div className="flex-1 max-w-md">
              ...
              <p className="text-sm text-text-secondary leading-relaxed mb-4">
                No manual refresh needed. When someone updates a task in Linear, it appears on your timeline within half
                a minute. Export as PNG or PDF for your next standup.
              </p>
            </div>
            <div className="flex-1 rounded-lg border border-border-primary bg-bg-card p-6 flex flex-col gap-3">
              ...
              <div className="flex gap-2">
                <div className="flex-1 text-center py-2 rounded border border-border-primary text-[10px] text-text-secondary hover:border-accent hover:text-accent transition-colors cursor-default">
                  PNG
                </div>
                <div className="flex-1 text-center py-2 rounded border border-border-primary text-[10px] text-text-secondary hover:border-accent hover:text-accent transition-colors cursor-default">
                  PDF
      ...
      <section className="py-24 sm:py-32 px-6">
        <div className="max-w-[600px] mx-auto text-center">
          ...
          <p className="text-sm text-text-muted mb-8">Connect in 10 seconds. No vendor lock-in, no hidden fees.</p>
          <Link
            to="/app"
